  ...
  return (
    <MemberShell user={access.user} demo={!access.decision.isMemberAccess || viewer.isDemo} title={MERIDIAN_INDEX}>
      ...
      <nav className="mt-8 flex gap-2 border-b border-[var(--line)]" aria-label="Index sections">
        <Tab href="/member/index" on={tab === "for-you"}>
          {INDEX_TAB_FOR_YOU}
        </Tab>
        <Tab href="/member/index?tab=circle" on={tab === "circle"}>
          {INDEX_TAB_CIRCLE}
        </Tab>
        <Tab href="/member/index?tab=all" on={tab === "all"}>
          {INDEX_TAB_ALL}
        </Tab>
      </nav>
      ...
      <div className="mt-6">
        {tab === "for-you" ? (
          index.meridian10.length === 0 ? (
            <p className="text-sm text-[var(--ivory-dim)]">No one in this frame. Profiles are never invented.</p>
          ) : (
