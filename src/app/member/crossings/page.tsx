...
export default async function CrossingsPage() {
  ...
  const mine = journeys.filter(
    (j) => j.profileId === viewer.id && j.status !== "deleted",
  );
  ...
  const tables = tableSuggestionsFor(
    store.crossings,
    store.profiles,
    viewer.id,
  );
  ...
  const prefs = store.crossings.prefs.find(
    (p) => p.profileId === viewer.id,
  ) ?? {
    profileId: viewer.id,
  ...
  return (
    <MemberShell user={access.user} demo title="Crossings">
      <div className="crossings-hero" data-reveal>
        <p className="label">Where paths meet</p>
        <h1 className="mt-4 max-w-2xl font-serif">
          Good company.
          <br />
          <em className="text-gold">New coordinates.</em>
        </h1>
        <p className="mt-3 max-w-lg text-ivory-muted">{CROSSINGS_COPY.line}</p>
        {canMutate ? (
          <div className="mt-6">
            <Button href="/member/crossings/new">
              {CROSSINGS_COPY.createAction}
            </Button>
          </div>
        ) : (
          <p className="mt-6 text-sm text-gold">
            Open House walkthrough — SYNTHETIC DEMO only. Active members set
            coordinates.
          </p>
        )}
      </div>

      <nav className="crossings-links mt-7 grid grid-cols-3 gap-3">
        <Link href="/member/crossings/notes" className="crossings-link">
          <p className="label">City Notes</p>
          <p className="mt-2 font-serif text-2xl">A private guide</p>
        </Link>
        <Link href="/member/crossings/hosts" className="crossings-link">
          <p className="label">City Hosts</p>
          <p className="mt-2 font-serif text-2xl">A member welcome</p>
        </Link>
        <Link href="/member/crossings/tables" className="crossings-link">
      ...
      {tables.length ? (
        <section className="mt-10">
          ...
          {tables.map((t) => (
            <article
              key={`${t.city}-${t.country}`}
              className="editorial-row mt-3"
            >
              <p className="font-serif text-2xl">
                {t.count} paths cross in {t.city}.
              </p>
              <p className="mt-2 text-sm text-ivory-muted">
                A private group meal. Neighborhood, not a public venue.
              </p>
              {canMutate ? (
                <Link
