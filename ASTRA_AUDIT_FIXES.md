...
1. **DM destination** — Message from a profile or My Circle opens `/member/messages?to={id}&channel={dm}`. The inbox never falls back to `#introductions`. If the DM cannot load, the header still names the member, compose is disabled, and Retry is explicit. Destination stays in the URL across refresh and back.
...
Nav is **Home · My Circle · Messages · Crossings · Profile**. My Circle (`/member/circle`; `/member/index` redirects) holds For you / Your Circle / All members. For you has a Meridian 10–100 size control. Messages holds DMs + Channels. Header search + notification bell. Desktop left rail uses the same names. Nothing is called “Matches”.
...
Copy is shortened: lock is centered headline + countdown + Remind me on a grainy black-and-gold field; Open House is hero, three benefits, experiences, $10,000 lifetime, no-soliciting; Home leads with connections and the next trip/experience; My Circle reasons are one line + city. Referral Rewards show **points** (10 pts = $1,000).
